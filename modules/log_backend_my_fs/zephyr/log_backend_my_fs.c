 ...
 *
 * Edited from /opt/nordic/ncs/v1.7.1/zephyr/subsys/logging/log_backend_fs.c
 */
...
#define LOG_PREFIX_LEN (sizeof(CONFIG_LOG_BACKEND_MY_FS_FILE_PREFIX) - 1)
...
static int check_log_volumen_available(void)
{
    ...
    while (rc == 0) {
        ...
        if (rc == 0) {
            if (strncmp(CONFIG_LOG_BACKEND_MY_FS_DIR,
                    name,
                    strlen(name))
                == 0) {
                return 0;
...
static int check_log_file_exist(int num)
{
    ...
    rc = fs_opendir(&dir, CONFIG_LOG_BACKEND_MY_FS_DIR);
    if (rc) {
        return -EIO;
    }
...
int write_log_to_file(uint8_t *data, size_t length, void *ctx)
{
    ...
    if (backend_state == BACKEND_FS_NOT_INITIALIZED) {
        ...
        rc = create_log_dir(CONFIG_LOG_BACKEND_MY_FS_DIR);
        if (!rc) {
    ...
    if (backend_state == BACKEND_FS_OK) {
        ...
        if (size < 0) {
            ...
            return length;
        } else if ((size + length) > CONFIG_LOG_BACKEND_MY_FS_FILE_SIZE) {
        ...
        if (rc >= 0) {
            if (IS_ENABLED(CONFIG_LOG_BACKEND_MY_FS_OVERWRITE) &&
                (rc != length)) {
                del_oldest_log();
...
static int get_log_file_id(struct fs_dirent *ent)
{
    ...
    if (memcmp(ent->name, CONFIG_LOG_BACKEND_MY_FS_FILE_PREFIX, LOG_PREFIX_LEN) != 0) {
        return -1;
    }
...
static int allocate_new_file(struct fs_file_t *file)
{
    ...
    if (backend_state == BACKEND_FS_NOT_INITIALIZED) {
        ...
        if ((file_ctr > 1) &&
            ((max - min) > 2 * CONFIG_LOG_BACKEND_MY_FS_FILES_LIMIT)) {
            ...
            while (rc == 0) {
                ...
                file_num = get_log_file_id(&ent);
                if (file_num < min + CONFIG_LOG_BACKEND_MY_FS_FILES_LIMIT) {
                    if (newest < file_num) {
                        newest = file_num;
                    }
                }

                if (file_num > max - CONFIG_LOG_BACKEND_MY_FS_FILES_LIMIT) {
                    if (oldest > file_num) {
                        oldest = file_num;
                    }
                }
    ...
    rc = fs_statvfs(CONFIG_LOG_BACKEND_MY_FS_DIR, &stat);

    /* Check if there is enough space to write file or max files number
     * is not exceeded.
     */
    while (file_ctr >= CONFIG_LOG_BACKEND_MY_FS_FILES_LIMIT) {
        if (IS_ENABLED(CONFIG_LOG_BACKEND_MY_FS_OVERWRITE)) {
            rc = del_oldest_log();
            if (rc < 0) {
                goto out;
    ...
    snprintf(fname, sizeof(fname), "%s/%s%04d",
        CONFIG_LOG_BACKEND_MY_FS_DIR,
        CONFIG_LOG_BACKEND_MY_FS_FILE_PREFIX, curr_file_num);
...
static int del_oldest_log(void)
{
    ...
    while (true) {
        snprintf(dellname, sizeof(dellname), "%s/%s%04d",
             CONFIG_LOG_BACKEND_MY_FS_DIR,
             CONFIG_LOG_BACKEND_MY_FS_FILE_PREFIX, oldest);
